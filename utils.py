...
def interpolate(t, track):
    ...
    if (len(before_p) > 0) and (len(after_p) > 0):
        ...
        try:
            ...
            course_interp = ( ((track[apos,COG] - track[bpos,COG])+180) % 360 - 180 )*(dt_interp/dt_full) + track[bpos,COG]
            heading_interp = ( ((track[apos,HEADING] - track[bpos,HEADING])+180) % 360 - 180 )*(dt_interp/dt_full) + track[bpos,HEADING]  
